...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if update.is_valid():
            ...
            messages.success(request, f'congratulations {{new}}, Your profile has been update successful!.....')
...
def password(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = form.save()
            messages.success(request, 'Password Chnage Successfull!.....')
            return redirect('profile')
        else:
            messages.error(request, form.errors)
...
def deleteitem(request):
  if request.method == 'POST':  
    ...
    item_delete.delete()
    messages.success(request, 'Cart delete successfulL!....')
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_1465911f9efc0799279dd1336e4dc5b78154ec85'
        curl = 'https://api.paystack.co/transaction/initialize'
        cburl = 'http://52.55.15.51/callback/'
...
def history(request):
    ...
    # categories = Category.objects.get() to get a particular object
    ...
    # objects managers
    # query methods
    # return HttpResponse('Hello Team Cyber Dev')
